...
def scrape_listing(url, listing_type):
    ...
    try:
        ...
        # Contact Info - extract from contact_name and page content
        contact_info = {
            "nombre": "",
            "correo": "",
            "telefono": "",
        ...
        # Extract seller name from contact_name link
        contact_name_el = soup.select_one("a.contact_name")
        if contact_name_el:
            contact_info["nombre"] = contact_name_el.get_text(strip=True)
        else:
            # Fallback to other selectors
            contact_section = soup.select_one(".d3-property-contact__name, .seller-name, .contact-name")
            if contact_section:
                contact_info["nombre"] = contact_section.get_text(strip=True)
        
        # Extract WhatsApp phone from links (href contains api.whatsapp.com/send/?phone=)
        page_html = str(soup)
        whatsapp_match = re.search(r'whatsapp\.com/send/?\?phone=(\d+)', page_html)
        if whatsapp_match:
            phone = whatsapp_match.group(1)
            # Keep full phone number with country code
            contact_info["whatsapp"] = phone
            if not contact_info["telefono"]:
                contact_info["telefono"] = phone
        
        # Extract phone from tel: links (keep full number)
        tel_match = re.search(r'href=["\']tel:(\+?\d+)["\']', page_html)
        if tel_match:
            phone = tel_match.group(1).replace("+", "")
            contact_info["telefono"] = phone
        
        # Also check data-contact attributes and script content for phone numbers
        for script in soup.select("script"):
            script_text = script.string or ""
            # Look for WhatsApp URLs in scripts
            wa_in_script = re.search(r'whatsapp\.com/send/?\?phone=(\d+)', script_text)
            if wa_in_script and not contact_info["whatsapp"]:
                phone = wa_in_script.group(1)
                contact_info["whatsapp"] = phone
                if not contact_info["telefono"]:
                    contact_info["telefono"] = phone
        
        # Fallback: Extract phone numbers from page text (El Salvador format)
        if not contact_info["telefono"]:
            page_text = soup.get_text()
            # Match full phone with optional country code
            phone_patterns = re.findall(r'(?:\+?503)?[\s-]?([267]\d{3}[\s-]?\d{4})', page_text)
            if phone_patterns:
                cleaned = re.sub(r'[\s-]', '', phone_patterns[0])
                if cleaned != external_id:
                    # Add country code if not present
                    if len(cleaned) == 8:
                        cleaned = "503" + cleaned
                    contact_info["telefono"] = cleaned
                    if cleaned[-8:].startswith(('7', '6')):
                        contact_info["whatsapp"] = cleaned
        ...
        for email in email_patterns:
            if not any(x in email.lower() for x in ['encuentra24', 'noreply', 'support', 'info@']):
                contact_info["correo"] = email
                break
...
def scrape_micasasv_listing(url, listing_type):
    ...
    try:
        ...
        if tel_link:
            phone = tel_link.get("href", "").replace("tel:", "").replace("+", "")
            # Decode URL-encoded characters and remove spaces
            phone = unquote(phone).replace(" ", "").replace("-", "")
            contact_info["telefono"] = phone
            # Check if it's a mobile (WhatsApp)
            if phone and (phone.startswith("503") and len(phone) >= 11):
                last_8 = phone[-8:]
                if last_8.startswith(("6", "7")):
                    contact_info["whatsapp"] = phone
...
def get_realtor_listings_from_page(page_url):
    ...
    try:
        ...
        for key, value in apollo_state.items():
            ...
            # Resolve agents for contact info
            agents = value.get("agents", [])
            contact_info = {}
            if agents:
                agent_data = resolve_ref(agents[0])
                if isinstance(agent_data, dict):
                    contact_info["nombre"] = agent_data.get("name", "")
                    contact_info["telefono"] = agent_data.get("officePhone", "").replace("+503 ", "503").replace(" ", "")
                    contact_info["agencia"] = agent_data.get("agencyName", "")
                    # Resolve contact for email
                    contact_ref = agent_data.get("contact")
                    contact_data = resolve_ref(contact_ref)
                    if isinstance(contact_data, dict):
                        contact_info["correo"] = contact_data.get("email", "")
